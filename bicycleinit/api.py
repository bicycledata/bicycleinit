...
def register(api_url, timeout=5):
    ...
    try:
        response = requests.post(
            f"{api_url}/api/v2/register", json=payload, timeout=timeout, headers={"Content-Type": "application/json"}
        )
    ...
    for session in sessions:
        ...
        try:
            for file in files:
                ...
                payload = {"ident": ident, "session": session, "path": "", "filename": upload_name, "data": data}
