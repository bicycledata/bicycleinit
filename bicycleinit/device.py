...
from bicycleinit import api, bluetooth, boxui, sensor_manager, upgrade, wifi, bicyclebutton
...
class BicycleDevice:
  ...
  def move_all_pending_files(self):
    ...
    for session in sessions:
      try:
        ...
        files = [name for name in os.listdir(os.path.join('temp', session)) if os.path.isfile(os.path.join('temp', session, name))]
        for file in files:
  ...
  def main(self):
    ...
    for sensor in sensors:
      upgrade.clone_or_pull_repo(sensor['git_url'], os.path.join('sensors', sensor['name']), sensor.get('git_branch', 'main'))
